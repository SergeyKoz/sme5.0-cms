 ...
 */
class Moxiecode_SMEngineAuthenticatorPlugin extends Moxiecode_ManagerPlugin {
	/**
	...
	function onInit(&$man) {
		...
		if (is_array($sme_session) && count($sme_session))
			foreach ($sme_session as $key => $value)
				$config[$key]=$value;
...
$man->registerPlugin("SMEngineAuthenticator", new Moxiecode_SMEngineAuthenticatorPlugin());
