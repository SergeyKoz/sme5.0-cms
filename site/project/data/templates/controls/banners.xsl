...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	<xsl:template match="bannerplace">
		...
		<xsl:for-each select="banner">
			<div class="bnr" rel="SME:&amp;#123;'mode':'banner', 'item_id':{banner_id}&amp;#125;">
				<xsl:choose>
					<!-- EXTERNAL BANNER -->
						...
						<xsl:if test="(banner_url != '') and (banner_file != '')">
							<a>
								...
								<xsl:if test="not(starts-with(banner_url, '#'))">
									<xsl:attribute name="href"><xsl:value-of select="/page/@url"/>scripts/r.php?b_id=<xsl:value-of select="banner_id"/>&amp;p_id=<xsl:value-of select="/page/@id"/>&amp;pl_id=<xsl:value-of select="$place_id"/>&amp;l=<xsl:value-of select="/page/@language"/></xsl:attribute>
								</xsl:if>
					...
					<xsl:when test="banner_type = 2">
						...
						<xsl:if test="(banner_file != '') and (width &gt; 0) and (height &gt; 0)">
							<OBJECT classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=5,0,0,0" BORDER="0" WIDTH="{width}" HEIGHT="{height}">
								<PARAM NAME="movie" VALUE="{/page/settings/filestorageurl}{banner_file}"/>
								<PARAM NAME="quality" VALUE="high"/>
								<PARAM NAME="background" VALUE="transparent"/>
								<EMBED src="{/page/settings/filestorageurl}{banner_file}" quality="high" WIDTH="{width}" HEIGHT="{height}" BORDER="1" TYPE="application/x-shockwave-flash" PLUGINSPAGE="http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash"/>
							</OBJECT>
						</xsl:if>
					</xsl:when>
					...
					<xsl:when test="banner_type = 3">
						...
						<xsl:if test="(banner_url != '') and (banner_alt != '')">
							<a href="{/page/@url}scripts/r.php?b_id={banner_id}&amp;p_id={/page/@id}&amp;pl_id={$place_id}&amp;l={/page/@language}">
								<xsl:if test="target = 1">
									<xsl:attribute name="target">_blank</xsl:attribute>
								</xsl:if>
