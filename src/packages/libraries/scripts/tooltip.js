...
function tt_Htm(tt, t_id, txt)
{
	...
	t_y += '<table cellpadding="0" cellspacing="0" border="0" width="100%"><tr><td background="'+ipath+'info_bg.gif"><img src="'+ipath+'info_bg.gif" width="10" height="5"/></td><td><img src="'+ipath+'info_corner.gif" width="7" height="5"/></td><td width="99%" background="'+ipath+'info_bg.gif"><img src="'+ipath+'info_bg.gif" width="1" height="5"/></td><td><img src="'+ipath+'zero.gif" width="3" height="5"/></td></tr></table>';
	...
	t_y += '<tr><td><table border="0" cellpadding="' + t_padd + '" cellspacing="0" width="100%" class="infoTable">';
	...
	if (t_shw)
	{
		...
		else
		{
			...
			t_y += '<div id="' + t_id + 'R" style="position:absolute;background:' + t_shc + ';left:' + t_w + 'px;top:' + (t_spct+4) + 'px;width:' + t_shw + 'px;height:1px;overflow:hidden;' + t_opa + '"><\/div>';
	...
	var i = tt_tags.length; while (i--)
	{
		...
		j = tags.length; while (j--)
		{
			if (typeof (t_tj = tags[j]).onmouseover == 'function' && t_tj.onmouseover.toString().indexOf(esc) != -1 && !tt_n6 || tt_n6 && (over = t_tj.getAttribute('onmouseover')) && over.indexOf(esc) != -1)
			{
				...
				if(i)window.globalVariableI = i;
				if(j)window.globalVariableJ = j;
...
function tt_ShowOnLoad()        {
				if(window.globalVariableI)ii=window.globalVariableI;
				if(window.globalVariableJ)jj=window.globalVariableJ;
				tt_obj = tt_GetDiv('tOoLtIp'+ii+''+jj+'');
				tt_SetDivPos(tt_Int(561), tt_Int(74));
        if (tt_n4) tt_obj.visibility = 'show';
        else tt_obj.style.visibility = 'visible';
        tt_vis = true;
        tt_HideInput();
        
        window.setTimeout('tt_Hide();',5000);   
}
